...
final class SettingsViewController: UITableViewController {
    ...
    private enum Row: Int, CaseIterable {
        case ads, popups, redirects, trackers, suspiciousURLs, privateMode, https, dns, videos, downloads, lowMemory
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .dns: configureSwitch(cell, title: "Encrypted DNS", on: s.secureDNS, tag: item.rawValue)
    ...
    @objc private func toggle(_ sw: UISwitch) {
        ...
        case .dns:
            setDNS(enabled: sw.isOn, control: sw)
            return
    ...
    private func setDNS(enabled: Bool, control: UISwitch) {
        if enabled {
            DNSManager.shared.enableDoH(urlString: s.dnsURL) { [weak self] error in
                DispatchQueue.main.async {
                    guard let self else { return }
                    if let error {
                        self.s.secureDNS = false
                        control.setOn(false, animated: true)
                        self.showError(error)
                    } else {
                        self.s.secureDNS = true
                        self.onChanged?()
                    }
                }
            }
        } else {
            DNSManager.shared.disable { [weak self] error in
                DispatchQueue.main.async {
                    guard let self else { return }
                    if let error {
